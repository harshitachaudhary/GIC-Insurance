...
public partial class nyksi_panel_Upload_Main_Product : System.Web.UI.Page
{
    ...
    {
        con.Open();
        SqlCommand cmd = new SqlCommand("select * from tblaboutus where id=@id", con);
    ...
    protected void Button1x_Click(object sender, EventArgs e)
    {
        ...
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("update tblaboutus set heading=@heading,para_1=@para_1,para_2=@para_2 where id=@id", con);
            cmd.CommandType = CommandType.Text;
            cmd.Parameters.AddWithValue("@id", "1");
            ...
            if(i > 0)
            {
                ...
                ScriptManager.RegisterStartupScript(this, GetType(), "Message", "alert('About Content Updated Successfully');", true);
            }
        }
        catch { }
        finally { con.Close(); }
