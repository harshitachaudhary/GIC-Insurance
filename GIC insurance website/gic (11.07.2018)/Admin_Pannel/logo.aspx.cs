...
public partial class nyksi_panel_Upload_Main_Product : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if(!IsPostBack)
        {
            if (Session["gic"] != null)
            {
                ...
                logoimage.ImageUrl = all.showpagecontent("logo");
    ...
    protected void Button1x_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            { strfile1 = logoimage.ImageUrl; }


            SqlCommand cmd = new SqlCommand("proc_tblallpages", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@typee", "logo");
            cmd.Parameters.AddWithValue("@content", strfile1);
            int i = cmd.ExecuteNonQuery();
            if (i > 0)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "Message", "alert('Logo Updated Successfully');", true);
            }
            logoimage.ImageUrl = all.showpagecontent("logo");
