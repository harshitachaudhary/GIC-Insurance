...
public partial class nyksi_panel_banner : System.Web.UI.Page
{
    ...
    {
        SqlCommand cmd = new SqlCommand("select *from tblservice_details", con);
    ...
    protected void lb_Edit_Click(object sender, EventArgs e)
    {
        ...
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("delete from tblservice_details where id=@id", con);
            cmd.CommandType = CommandType.Text;
            ...
            if (i > 0)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Service Successfully Deleted');location.href='services-list.aspx'", true);
            }
