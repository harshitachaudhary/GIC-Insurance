...
public partial class nyksi_panel_banner : System.Web.UI.Page
{
    ...
    protected void lbdelete_Click(object sender, EventArgs e)
    {
        ...
        if (i > 0)
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Successfully Deleted');location.href='gic-list.aspx'", true);
        }
        con.Close();
    }
    ...
    {
        string s = "select * from tblgic where name like '%" + txtsearchbyname.Text + "%'";
    ...
    {
        string s = "select * from tblgic where vehicle_no like '%" + txtsearchbyvehicleno.Text + "%'";
        SqlCommand cmd = new SqlCommand(s, con);
        SqlDataAdapter da = new SqlDataAdapter(cmd);
        DataTable dt = new DataTable();
        da.Fill(dt);
