...
public partial class nyksi_panel_Upload_Main_Product : System.Web.UI.Page
{
    ...
    protected void btnsubmit_Click1(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd.Parameters.AddWithValue("@chassis_no", txtchassisno.Text);
            cmd.Parameters.AddWithValue("@name", txtname.Text);
            cmd.Parameters.AddWithValue("@expiry_date", txtexpirydate.Value);
            cmd.Parameters.AddWithValue("@rupees", txtrs.Text);
            cmd.Parameters.AddWithValue("@mobile_no", txtmobileno.Text);
            cmd.Parameters.AddWithValue("@var", "ins");
            int i = cmd.ExecuteNonQuery();
            if (i > 0)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Successfully Submited');location.href='insert-gictax-details.aspx'", true);
            }
            else
            {
